  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      <aside
        ...
        className="absolute inset-y-0 right-0 flex w-full max-w-3xl flex-col overflow-y-auto border-l border-grit-glass-border bg-grit-card shadow-[0_18px_44px_rgba(0,0,0,0.45)]"
      >
        ...
        <div className="grid flex-1 grid-cols-1 gap-6 px-5 py-4 md:grid-cols-2">
          <div>
            <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-grit-subtext">Vista previa</p>
            <PublicTrainingCard data={previewData} reservarDisabled />
          </div>

          <div className="space-y-4">
            <div>
              <label htmlFor="publicacion-nombre" className="mb-1 block text-xs text-grit-subtext">
                Nombre de la publicación <span className="text-grit-danger">*</span>
              </label>
              <input
            ...
            <div>
              <label htmlFor="publicacion-descripcion" className="mb-1 block text-xs text-grit-subtext">
                Descripción
              </label>
              <textarea
            ...
            <div>
              <label htmlFor="publicacion-descripcion-larga" className="mb-1 block text-xs text-grit-subtext">
              ...
              />
              <p className="mt-1 text-[11px] text-grit-muted">
                Se admite Markdown y se muestra en la página pública del entrenamiento.
              </p>
            </div>

            <div>
              <label htmlFor="publicacion-pagina-evento" className="mb-1 block text-xs text-grit-subtext">
              ...
              />
              {paginaEventoUrlError ? (
                <p className="mt-1 text-xs text-grit-danger">{paginaEventoUrlError}</p>
              ) : (
                <p className="mt-1 text-[11px] text-grit-muted">
            ...
            <fieldset className="space-y-2 rounded-grit-md border border-grit-glass-border p-3">
              <legend className="px-1 text-xs font-semibold text-grit-subtext">Precios y opciones</legend>
              <p className="text-[11px] text-grit-muted">Sin filas se muestra &quot;Gratis&quot;.</p>
              {values.precio.map((row, index) => (
                <div key={index} className="space-y-1 rounded-md border border-grit-glass-border p-2">
                  <div className="flex gap-2">
                    <input
                      ...
                      className="w-1/2 rounded-grit-md border border-grit-glass-border bg-grit-bg px-2 py-1.5 text-sm text-grit-text"
                    />
                    <input
                      type="number"
                      min={0}
                      ...
                      disabled={isLoading || isSubmitting}
                      placeholder="COP"
                      className="w-1/3 rounded-grit-md border border-grit-glass-border bg-grit-bg px-2 py-1.5 text-sm text-grit-text"
                    />
                    <button
                      type="button"
                      ...
                      disabled={isLoading || isSubmitting}
                      className="rounded-grit-md border border-grit-glass-border px-2 text-grit-subtext transition hover:text-grit-danger"
                    >
                      <span className="material-symbols-outlined text-base" aria-hidden="true">
                        delete
                      </span>
                  ...
                  <input
                    ...
                    className="w-full rounded-grit-md border border-grit-glass-border bg-grit-bg px-2 py-1.5 text-sm text-grit-text"
                  />
                  {precioErrors[index] && <p className="text-xs text-grit-danger">{precioErrors[index]}</p>}
                </div>
              ))}
              <button
                type="button"
                ...
                disabled={isLoading || isSubmitting}
                className="rounded-grit-md border border-grit-glass-border px-3 py-1.5 text-xs font-semibold text-grit-text transition hover:border-grit-glass-border"
              >
                Añadir precio
              </button>
            </fieldset>
            ...
            <fieldset className="space-y-2 rounded-grit-md border border-grit-glass-border p-3">
              <legend className="px-1 text-xs font-semibold text-grit-subtext">Cronograma</legend>
              <p className="text-[11px] text-grit-muted">El orden de las filas es el orden que se muestra.</p>
              {values.cronograma.map((row, index) => (
                <div key={index} className="flex gap-2">
                  ...
                  <input
                    ...
                    className="flex-1 rounded-grit-md border border-grit-glass-border bg-grit-bg px-2 py-1.5 text-sm text-grit-text"
                  />
                  <button
                    type="button"
            ...
            <fieldset className="space-y-2 rounded-grit-md border border-grit-glass-border p-3">
              <legend className="px-1 text-xs font-semibold text-grit-subtext">¿Qué incluye?</legend>
            ...
            <div>
              <label htmlFor="publicacion-banner" className="mb-1 block text-xs text-grit-subtext">
              ...
              <input
                ...
                disabled={isSubmitting}
                className="w-full text-sm text-grit-subtext file:mr-3 file:rounded-grit-md file:border-0 file:bg-grit-bg file:px-3 file:py-2 file:text-sm file:text-grit-text"
              />
              {bannerError && <p className="mt-1 text-xs text-grit-danger">{bannerError}</p>}
            </div>

            <div className="rounded-grit-md border border-grit-glass-border bg-grit-bg/50 p-3 text-xs text-grit-subtext">
              <p className="mb-1 font-semibold text-grit-subtext">Datos fijos del entrenamiento (no editables aquí)</p>
              <p>Fecha y hora, duración, escenario y cupo se sincronizan siempre con el entrenamiento original.</p>
            </div>

            <div className="space-y-1.5">
              <span id="omitir-confirmacion-plan-desc" className="block text-[11px] text-grit-muted">
              ...
              <label className="flex cursor-pointer items-center gap-3">
                ...
                />
                <span className="text-sm font-medium text-grit-text">Omitir confirmación de plan</span>
              </label>
            </div>
        ...
        <footer className="flex items-center justify-end gap-3 border-t border-grit-glass-border px-5 py-4">
          {isPublished && (
            <button
              ...
              disabled={isSubmitting}
              className="mr-auto rounded-grit-md border border-grit-danger/40 bg-rose-500/10 px-4 py-2 text-sm font-semibold text-grit-danger transition hover:border-grit-danger/70 disabled:cursor-not-allowed disabled:opacity-60"
            >
              Quitar publicación
            </button>
          )}
