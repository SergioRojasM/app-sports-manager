...
// Explicit 8px radius (not the lg/xl radius utilities — overridden in this project to 2rem/3rem for the
...
function FormularioCampoEditableField({
  ...
  if (campoTipo === 'checkbox') {
    return (
      <div>
        <label className="flex items-center gap-2 text-sm font-medium text-grit-subtext">
          <input
            type="checkbox"
            checked={value === 'true'}
            disabled={disabled}
            onChange={(e) => onUpdateValue(campoNombre, e.target.checked ? 'true' : 'false')}
            className="rounded border-grit-glass-border bg-grit-bg/60"
          />
          {seccion.campo_etiqueta}
          {seccion.campo_obligatorio && <span className="text-grit-danger"> *</span>}
        </label>
        {error && <p className="mt-1 text-xs text-grit-danger">{error}</p>}
      </div>
    );
  }
  ...
  return (
    <div>
      <label className="mb-1 block text-sm font-medium text-grit-subtext">
        {seccion.campo_etiqueta}
        {seccion.campo_obligatorio && <span className="text-grit-danger"> *</span>}
      </label>
      ...
      {campoTipo === 'seleccion' ? (
            ...
            return (
              <button
                ...
                className={[
                  ...
                  selected
                    ? 'border-grit-cyan bg-grit-cyan/15 text-grit-cyan'
                    : 'border-grit-glass-border bg-grit-bg/60 text-grit-subtext hover:border-grit-cyan/50',
                ].join(' ')}
              >
                {opcion}
              </button>
        ...
        <div>
          <input
            ...
            className={inputClass}
          />
          {uploading && <p className="mt-1 text-xs text-grit-subtext">Subiendo archivo…</p>}
    ...
    >
      <div className="border bg-grit-glass backdrop-blur-md flex max-h-[85vh] w-full max-w-2xl flex-col overflow-hidden rounded-grit-2xl border-grit-glass-border bg-grit-card shadow-[0_18px_44px_rgba(0,0,0,0.45)]">
        ...
        {plantillaHeaderSecciones.length > 0 ? (
          ...
          <h2 className="font-grit-title px-6 pt-6 text-lg font-semibold text-grit-text">{plantillaNombre}</h2>
        )}

        <div className="p-6 pt-4">
        <p className="mb-4 text-sm text-grit-subtext">Completa el formulario para continuar con tu reserva.</p>

        {headerExtra}

        {perfilIncompleto ? (
          <div className="mb-4 rounded-grit-md border border-amber-400/40 bg-amber-500/15 px-4 py-3 text-sm text-amber-200">
            <p>
          ...
          <div className="mb-4 rounded-[8px] border border-grit-glass-border bg-grit-bg/60 px-3 py-2.5">
            <p className="mb-1.5 text-xs text-grit-subtext">Estos datos de tu perfil se usarán en esta reserva:</p>
            <div className="flex flex-wrap gap-1.5">
              {perfilResumen.map((item) => (
                <span
                  key={item.key}
                  className="inline-flex items-center gap-1 rounded-[8px] border border-grit-cyan/25 bg-grit-cyan/10 px-2 py-1 text-xs font-medium text-grit-cyan"
                >
                  <span className="text-grit-subtext">{item.label}:</span> {item.value}
                </span>
              ))}
            </div>
          </div>
        ...
        <div className="space-y-4">
          {loading && <p className="text-sm text-grit-subtext">Cargando formulario…</p>}
          ...
          {!loading && !loadError && secciones.length === 0 && (
            <p className="text-sm text-grit-subtext">Este formulario todavía no tiene secciones.</p>
          )}
        ...
        <div className="border-t border-grit-glass-border p-6 pt-4">
          <p className="mb-3 text-[11px] leading-relaxed text-grit-muted">
