...
export function ReservasManagementTable({
  rows,
  currentPage,
  pageSize,
  totalPages,
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-portal-border">
            {rows.length === 0 ? (
              ...
              rows.map((row) => {
                const fullName = [row.atleta_nombre, row.atleta_apellido].filter(Boolean).join(' ') || '—';
                const secondLine = [row.atleta_email, row.numero_identificacion].filter(Boolean).join(' · ');
