...
function initialRange(): AnaliticaDateRange {
  ...
  return { dateFrom: date.toISOString().slice(0, 10), dateTo, preset: '30d' };
}
...
export function AnaliticaPage({ tenantId }: { tenantId: string }) {
  ...
  const validationMessage = dateRange.dateFrom && dateRange.dateTo && dateRange.dateFrom > dateRange.dateTo
  ...
  return (
    <section className="space-y-6">
      <header>
        <h1 className="text-3xl font-semibold text-slate-100">Analítica</h1>
        <p className="mt-2 text-sm text-slate-400">Indicadores de ingresos, operación y equipo con fechas de Colombia.</p>
      </header>
      <AnaliticaDateRangeFilter value={dateRange} onChange={setDateRange} onRefresh={() => { if (!validationMessage) void refresh(); }} refreshing={refreshing} validationMessage={validationMessage} />
...
function Resumen({ data, kpis, onNavigate }: { data: AnaliticaDashboard; kpis: Array<[string, string, AnaliticaTab]>; onNavigate: (tab: AnaliticaTab) => void }) {
  return <div className="space-y-6">
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{kpis.map(([label, value, tab]) => <button key={label} type="button" onClick={() => onNavigate(tab)} className="text-left"><AnaliticaKpiCard label={label} value={value} /></button>)}</div>
    <div className="grid gap-4 lg:grid-cols-2">
      <Panel title="Ingresos por mes"><MiniBars items={data.revenue.monthlyRevenue.map((row) => ({ label: row.monthKey, value: row.recognizedRevenue, text: currency.format(row.recognizedRevenue) }))} /></Panel>
      <Panel title="Alertas de capacidad"><Alerts rows={data.operations.upcomingCapacityAlerts} /></Panel>
    </div>
  </div>;
}
...
function Panel({ title, children }: { title: string; children: React.ReactNode }) { return <section className="rounded-lg border border-portal-border bg-navy-deep/60 p-4"><h2 className="text-sm font-semibold text-slate-100">{title}</h2><div className="mt-4">{children}</div></section>; }
