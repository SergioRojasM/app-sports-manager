...
export function FormularioHeaderEditor({ tenantId, secciones, onUpdateHeaderField, readOnly = false }: FormularioHeaderEditorProps) {
  ...
  const editableTextClass =
    'w-full rounded-grit-md border border-transparent bg-transparent px-2 py-1 text-center outline-none transition hover:border-grit-glass-border focus:border-grit-cyan focus:bg-grit-bg focus:ring-2 focus:ring-grit-cyan/35';

  return (
    <div className="flex flex-col items-center gap-3.5 border-b border-grit-glass-border px-6 py-8 text-center">
      <div className="flex items-center gap-2">
        {tenantLogo?.logoUrl ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={tenantLogo.logoUrl} alt="" className="h-7 w-7 rounded-grit-md object-cover" />
        ) : (
          <span className="flex h-7 w-7 items-center justify-center rounded-grit-md bg-grit-cyan text-grit-bg">
            <span className="material-symbols-outlined text-base" aria-hidden="true">bolt</span>
          </span>
        )}
        <span className="text-[13px] font-bold tracking-[0.15em] text-grit-text">
      ...
      {readOnly || !onUpdateHeaderField ? (
        sobretitulo?.seccion_descripcion ? (
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-grit-cyan">{sobretitulo.seccion_descripcion}</p>
        ) : null
        ...
        <input
          ...
          className={`${editableTextClass} max-w-xs text-xs font-bold uppercase tracking-[0.2em] text-grit-cyan`}
        />
      ) : (
        <button
          type="button"
          onClick={() => setEditingField('sobretitulo')}
          className="rounded-grit-md px-2 py-1 text-xs font-bold uppercase tracking-[0.2em] text-grit-cyan transition hover:bg-grit-bg/60"
        >
      ...
      {readOnly || !onUpdateHeaderField ? (
        <h1 className="font-grit-title text-4xl font-bold leading-tight text-grit-text sm:text-5xl">{titulo.seccion_descripcion}</h1>
      ) : editingField === 'titulo' ? (
        <input
          autoFocus
          type="text"
          ...
          className={`${editableTextClass} max-w-xl text-4xl font-bold leading-tight text-grit-text sm:text-5xl`}
        />
      ) : (
        <button
          type="button"
          onClick={() => setEditingField('titulo')}
          className="rounded-grit-md px-2 py-1 text-4xl font-bold leading-tight text-grit-text transition hover:bg-grit-bg/60 sm:text-5xl"
        >
      ...
      <span
        aria-hidden="true"
        className="h-0.5 w-16 rounded-full bg-gradient-to-r from-transparent via-grit-cyan to-transparent"
      />

      {readOnly || !onUpdateHeaderField ? (
        subtitulo?.seccion_descripcion ? <p className="text-[15px] text-grit-subtext">{subtitulo.seccion_descripcion}</p> : null
        ...
        <input
          ...
          className={`${editableTextClass} max-w-lg text-[15px] text-grit-subtext`}
        />
      ) : (
        <button
          type="button"
          onClick={() => setEditingField('subtitulo')}
          className="rounded-grit-md px-2 py-1 text-[15px] text-grit-subtext transition hover:bg-grit-bg/60"
        >
