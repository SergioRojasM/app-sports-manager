...
│   │   │       ├── PublicTrainingCard.tsx           # Shared card (marketplace grid AND PublicarEntrenamientoModal's live preview), driven by PublicTrainingCardData; shows tenantNombre and a "Requiere: …" row from serviciosRequeridos (empty on the anonymous landing page — US-0094); "Ver" button over the banner (only when bannerUrl is set) opens PublicTrainingBannerModal fullscreen, local useState, no auth coupling (US-0100)
