...
│   │           ├── usePublicTrainingReserva.ts        # Thin composition of the EXISTING useReservaForm/useFormularioRespuestaForm for a cross-tenant booking; fetches formulario_id/formulario_externo directly from the source entrenamientos row (never duplicated); openBooking() runs validateBookingRestrictions as an upfront pre-check (checkingEligibility state) before opening the form, fails open on unexpected errors — the RPC-level check at submit remains authoritative (US-0101); accepts omitirConfirmacionPlan; continueWithPendingPlan(purchase: PendingPlanPurchaseDraft) stores the UNSAVED plan purchase in state and moves straight into the booking form, forwarding permitir_pendiente_sin_plan/plan_pendiente_compra into reservasService.create() so the suscripcion+pago are created in the same transaction as the reserva (US-0106, reworked by US-0110); after a successful booking it uploads any attached proof of payment (looked up via the returned reserva's suscripcion_id → pagos.id), best-effort in a try/catch — a failed upload never fails a committed booking, and the athlete can still resubmit via "Resubir comprobante"
