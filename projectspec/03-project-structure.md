...
│   │   │       │   ├── PublicTrainingDetalleUbicacion.tsx   # Escenario/ubicación/punto de encuentro + "Ver en Google Maps" search link; static map placeholder, no amenity tags (no data source)
│   │   │       │   ├── PublicTrainingDetalleReserva.tsx     # Cupos disponibles/duración/entrenador (entrenador row only when entrenadorNombre is set), primary "Reservar mi cupo" (disabled while useAuth() initializes) + "Ver detalles oficiales" gated on pagina_evento_url; no "Nivel recomendado" row (no data source)
...
│   │   │       ├── PublicarEntrenamientoModal.tsx  # Publish/manage-publication slide-over: live PublicTrainingCard preview + editable nombre/descripcion/precio/banner, "Despublicar" (US-0089); "Omitir confirmación de plan" checkbox lets a plan-blocked booking proceed as pending (US-0106); US-0109 replaces the single precio input with repeatable precio (nombre+COP+descripción, per-row inline validation), cronograma (hora+descripción) and incluye (título+descripción) row editors, plus a Markdown descripcion_larga textarea and a URL-validated pagina_evento_url field; only rows with a valid amount feed the live card preview
...
│   │           └── useSuscripcion.ts       # openModal auto-selects selectedTipoId when the plan has exactly one active subtype, via getActiveTipos (US-0105); optional onSubscribed callback fires after successful creation, skipping the generic success message for callers chaining their own next step (US-0106)
...
│   │           ├── useEntrenamientosPublicosMarketplace.ts  # Fetches listPublicTrainings + listPublicTenantOptions; client-side dateFrom/dateTo range + search/tenant filters, defaulting on mount to today→end of current month; owns calendarMonth + goToPrevMonth/goToNextMonth/setDateRange/clearDateRange/applyDateChip (chip toggles off when already-active, per its computed range); "this week" count independent of active filters (US-0102)
│   │           ├── usePublicTrainingReserva.ts        # Thin composition of the EXISTING useReservaForm/useFormularioRespuestaForm for a cross-tenant booking; fetches formulario_id/formulario_externo directly from the source entrenamientos row (never duplicated); openBooking() runs validateBookingRestrictions as an upfront pre-check (checkingEligibility state) before opening the form, fails open on unexpected errors — the RPC-level check at submit remains authoritative (US-0101); accepts omitirConfirmacionPlan; continueWithPendingPlan(suscripcionId) stores the pending plan and moves straight into the booking form, forwarding permitir_pendiente_sin_plan/plan_pendiente_suscripcion_id into reservasService.create() (US-0106)
...
│   │       └── entrenamientos-publicos.types.ts  # EntrenamientoPublico, PublicarEntrenamientoInput, PublicTrainingListItem (adds formularioId/formularioExterno, batched from the source entrenamientos row — US-0101), PublicTrainingFilters (dateFrom/dateTo/search/tenantId; PublicTrainingDateChip kept as the type for the four quick-chip presets — US-0102), EntrenamientoPublicoServiceError (codes incl. 'servicio_restriction') (US-0089); omitir_confirmacion_plan/omitirConfirmacionPlan on EntrenamientoPublico, PublicarEntrenamientoInput, EntrenamientoPublicoFormValues, PublicTrainingListItem (US-0106); US-0109 adds CronogramaItem/IncluyeItem/PrecioItem and changes precio from a scalar to PrecioItem[] on EntrenamientoPublico/PublicTrainingListItem (BREAKING), plus cronograma/incluye/descripcion_larga/pagina_evento_url and entrenadorNombre. EntrenamientoPublicoFormValues.precio is PrecioFormRow[] (amount kept as a raw string so an emptied input is rejected instead of coercing to 0 via Number('')); the hook converts to numeric PrecioItem[] on submit
│   │       └── reservas.types.ts         # ReservaView, CreateReservaInput, CategoriaDisponibilidad, ReservaReportRow (flat view type for CSV export, includes atleta_id), ReservasManagementFilters (server-side filter input — US-0073), MisReservasFilters (athlete-scoped filter input — US-0074; tenantId optional/cross-tenant, US-0097), ReservaReportRow.tenant_nombre (US-0097); Reserva.formulario_respuesta_id, CreateReservaInput.formulario_plantilla_id/formulario_respuesta (US-0087); ReservaEstado adds 'rechazada'; Reserva.motivo_rechazo, ReservaReportRow.motivo_rechazo; CreateReservaInput.permitir_pendiente_sin_plan/plan_pendiente_suscripcion_id (US-0106)
