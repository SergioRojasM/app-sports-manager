-- ============================================================================
-- bi.fct_suscripciones (bi-fct-suscripciones)
--
-- Subscription fact view: one row per public.suscripciones row, with plan,
-- plan type, Bogotá sale date, sold/active flags and a per-subscription
-- payment summary. Private like the other bi.* facts; clients only reach it
-- through get_tenant_bi_dashboard().
--
-- "Active" is estado = 'activa', the rule every portal service uses. The daily
-- vencer-suscripciones-diarias cron keeps it current, so dates are not
-- re-checked here.
--
-- get_tenant_bi_dashboard() is replaced with the 20260923120000 body; only the
-- subscriptions_sold and valid_subscriptions CTEs change, to read this view.
-- ============================================================================

begin;

create or replace view bi.fct_suscripciones as
select
  ...
  (s.created_at at time zone 'America/Bogota')::date as fecha_venta_analitica,
  s.fecha_inicio,
  s.fecha_fin,
  s.clases_plan,
  s.clases_restantes,
  s.estado <> 'cancelada' as es_vendida,
  s.estado = 'activa' as es_activa,
  coalesce(pg.pagos_count, 0) as pagos_count,
  coalesce(pg.pagos_validados_count, 0) as pagos_validados_count,
  coalesce(pg.monto_validado, 0) as monto_validado,
  coalesce(pg.monto_pendiente, 0) as monto_pendiente,
  coalesce(pg.monto_rechazado, 0) as monto_rechazado,
  pg.fecha_primer_pago_validado_analitica
...
left join (
  select
    ...
    count(*) filter (where p.estado = 'validado') as pagos_validados_count,
    sum(p.monto) filter (where p.estado = 'validado') as monto_validado,
    sum(p.monto) filter (where p.estado = 'pendiente') as monto_pendiente,
    sum(p.monto) filter (where p.estado = 'rechazado') as monto_rechazado,
    -- Same dating rule as bi.fct_pagos.fecha_pago_analitica.
    min((coalesce(p.fecha_validacion, p.fecha_pago, p.created_at) at time zone 'America/Bogota')::date)
      filter (where p.estado = 'validado') as fecha_primer_pago_validado_analitica
  from public.pagos p
  group by p.suscripcion_id
) pg on pg.suscripcion_id = s.id;

revoke all on bi.fct_suscripciones from public, anon, authenticated;
...
begin
  ...
  with member_states as (
    ...
    select distinct fs.atleta_id, fs.plan_tipo_id
...
create index if not exists idx_pagos_suscripcion_estado
  on public.pagos (suscripcion_id, estado);
