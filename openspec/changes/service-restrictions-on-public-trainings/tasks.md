...
- [ ] 9.1 End-to-end as a non-member: buy a public plan granting service X → admin validates the subscription → book a published training restricted to X → confirm the `reservas` row, the `suscripcion_servicios` deduction and the `reserva_servicios` ledger entry
- [ ] 9.2 Cancel that booking and confirm the units are restored
- [ ] 9.3 As a non-member with **no** subscription: confirm the rejection names the service and offers the catalog action, and that acquiring a plan from it follows the US-0093 flow
- [ ] 9.4 As a non-member whose subscription is still `pendiente`: confirm the booking is rejected
- [ ] 9.5 As a non-member with exhausted units: confirm the exhausted-units message and the catalog action
- [ ] 9.6 Marketplace: confirm the requirements row, the alphabetical distinct names, and that searching by a service name finds the session
- [ ] 9.7 Landing page as an anonymous visitor: confirm no requirements row, unchanged listing output, and the registration prompt on "Reservar"
- [ ] 9.8 Publish gate in the UI for each of the eight row combinations, matching the trigger results from 2.7
- [ ] 9.9 Regression: a member books a service-restricted training (public and private) with identical validation, deduction and messages
- [ ] 9.10 Regression: publish, un-publish (`activo = false`) and re-publish an unrestricted training
