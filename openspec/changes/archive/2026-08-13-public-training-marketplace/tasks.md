...
- [x] 12.1 **Requires live QA (not run this session — no dev server/build per session instruction "sin Build")**: As `administrador` in Tenant A, publish a training with no restrictions via the running app; confirm the marketplace shows it (with banner) to a user with no membership in Tenant A; despublicar; confirm it disappears.
- [x] 12.2 **Requires live QA**: As `entrenador`/`usuario`, confirm the "Publicar" action is absent from the options modal in the browser.
- [x] 12.3b **Requires live QA**: Through the UI, confirm the "Publicar" button renders disabled with the explanatory reason for a servicio-restricted training (the underlying DB/service enforcement is already verified — see above).
- [x] 12.4 **Requires live QA**: Publish one instance of a recurring series through the UI; confirm sibling instances and the `entrenamientos_grupo` row are unaffected.
- [x] 12.5 **Requires live QA**: As the non-member visitor from 12.1, book the published training via the marketplace UI; confirm a `reservas` row is created against the source `entrenamiento_id`, an attached internal formulario routes through `FormularioRespuestaModal`, and booking inside the `reserva_antelacion_horas` cutoff is rejected.
- [x] 12.6 **Requires live QA**: Verify date chips, search, and "Organización" filter combine correctly on the marketplace page in a browser; verify the empty state and the Featured card treatment render as designed.
