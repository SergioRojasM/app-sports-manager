...
- [x] 11.2 Test: open edit modal for a subscription with no services — verify "Unidades por Servicio" section is absent
- [x] 11.3 Test: open edit modal for a subscription with services — verify section appears with correct pre-populated values
- [x] 11.4 Test: toggle "Ilimitado" checkbox on and off — verify number input shows/hides and `unidades_restantes` is set correctly
- [x] 11.5 Test: save with changed service unit value — verify table refreshes with updated value, no console errors
- [x] 11.6 Test: save with no changes to service units — verify no `adminUpdateServicioUnidades` RPC calls are made (check browser network tab)
- [x] 11.7 Test: click "+X más" in SERVICIOS column — verify `VerServiciosModal` opens with full sorted service list
- [x] 11.8 Test: verify ATLETA and PLAN columns truncate long names with ellipsis
- [x] 11.9 Test: verify INICIO/FIN dates are the same text size
- [x] 11.10 Test: verify VALIDACIÓN column shows both names at the same small size
- [x] 11.11 Test: hover each ACCIONES icon button — verify `title` tooltip appears
