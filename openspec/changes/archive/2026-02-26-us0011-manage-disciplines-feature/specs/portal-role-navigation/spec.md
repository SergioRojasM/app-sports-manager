...
`PortalSidebar` SHALL display a tenant-discovery entry for authenticated users and SHALL adapt tenant-scoped menu items according to the resolved role for the active tenant context. Role-specific tenant items MUST only appear after tenant access is validated. The `gestion-disciplinas` tenant route entry SHALL be visible only for `administrador` role in the active tenant.
...
#### Scenario: Administrator sees disciplines-management route entry
- **WHEN** the resolved role for active tenant is `administrador`
- **THEN** the sidebar SHALL include a tenant-scoped entry for `gestion-disciplinas`
