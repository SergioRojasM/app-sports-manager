...
#### Scenario: Banner renders when bannerUrl is set
- **WHEN** `identity.bannerUrl` is non-null and the image loads successfully
- **THEN** an `<img>` with `src={bannerSrc}` and `alt="{orgName} banner"` SHALL fill the `h-24` header container

#### Scenario: Gradient shown when bannerUrl is null
- **WHEN** `identity.bannerUrl` is null
- **THEN** no `<img>` SHALL be rendered and the gradient background SHALL be visible

#### Scenario: Gradient shown after image load error and all fallbacks fail
- **WHEN** the banner `<img>` fires `onError` and all signed-URL refresh attempts fail
- **THEN** `bannerSrc` SHALL be set to null and the gradient SHALL be visible

#### Scenario: Placeholder Edit Banner button is removed
- **WHEN** `TenantIdentityCard` renders
- **THEN** there SHALL be no button with `aria-label="Edit organization banner (coming soon)"`

---

### Requirement: TenantInfoCards wires banner upload end-to-end
`TenantInfoCards` SHALL instantiate `useOrgBannerUpload()` and define `uploadBannerForTenant` (same pattern as `uploadLogoForTenant`). It SHALL pass `uploadBanner` to `useEditTenant` when `bannerUploadHook.selectedFile` is truthy. It SHALL pass the `bannerUpload` state object to `EditTenantDrawer` → `EditTenantForm`.

#### Scenario: Banner upload is wired when a file is selected
- **WHEN** `bannerUploadHook.selectedFile` is non-null
- **THEN** `useEditTenant` SHALL receive the `uploadBanner` callback

#### Scenario: Banner upload state is passed to the form
- **WHEN** `EditTenantDrawer` is rendered
- **THEN** the `bannerUpload` prop SHALL be passed with the hook's state

---

### Requirement: tenant.service.ts includes banner_url in all tenant queries and updates
`tenantService` functions that read or write tenant data SHALL include `banner_url` in their Supabase select strings. `getTenantViewData` (or equivalent) SHALL map `banner_url` to `TenantIdentityPayload.bannerUrl`. `fetchTenantEditData` SHALL include `banner_url` in `values.banner_url`. `updateTenant` SHALL include `banner_url: payload.banner_url ?? null` in the update object.

#### Scenario: View data includes bannerUrl
- **WHEN** `getTenantViewData` is called for a tenant with a non-null `banner_url`
- **THEN** the returned `identity.bannerUrl` SHALL equal the DB value

#### Scenario: Edit form initial values include banner_url
- **WHEN** `fetchTenantEditData` is called for a tenant with a non-null `banner_url`
- **THEN** `values.banner_url` SHALL equal the DB value

#### Scenario: updateTenant persists banner_url
- **WHEN** `updateTenant` is called with a payload containing a non-null `banner_url`
- **THEN** `tenants.banner_url` SHALL be updated to that value
