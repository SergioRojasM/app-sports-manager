...
- [ ] 4.1 Test InicioProximosEntrenamientos on 375px-wide viewport:
  - [ ] All training elements (icon, name, date, location, org, badge) visible without overlap
  - [ ] Metadata wraps to second line without truncation
  - [ ] Training row remains clickable
- [ ] 4.2 Test InicioProximosEntrenamientos on 640px viewport:
  - [ ] Layout matches current single-line design
  - [ ] No visual regression
- [ ] 4.3 Test SuscripcionModal step 1 (plan selection) on 375px × 667px viewport:
  - [ ] Modal displays with 3+ plan types
  - [ ] "Continuar" button is reachable by scrolling
  - [ ] Plan type cards remain clickable and selectable
- [ ] 4.4 Test SuscripcionModal step 2 (payment form) on 375px × 667px viewport:
  - [ ] Payment method select, comments textarea, file input are accessible
  - [ ] "Confirmar" button is reachable by scrolling
  - [ ] Form interaction works (fill fields, upload file, submit)
- [ ] 4.5 Test both components on desktop viewport (1024px+):
  - [ ] No visual regressions
  - [ ] Layout matches current design
- [ ] 4.6 Test modal backdrop behavior:
  - [ ] Clicking backdrop closes modal
  - [ ] Body scroll prevented when modal open

## 5. Accessibility and Interaction Verification

- [ ] 5.1 Verify training row `<Link>` still navigates correctly on click
- [ ] 5.2 Verify modal button click handlers still execute (Continuar, Confirmar, Cancelar)
- [ ] 5.3 Verify form submission works end-to-end on mobile (validate, upload, submit)
- [ ] 5.4 Test keyboard navigation (Tab through plan types, form fields, buttons)
...
- [ ] 6.2 Ensure no console errors or warnings in dev tools
- [ ] 6.3 Verify no unintended side effects on other pages or components

## 7. Commit and Pull Request

- [ ] 7.1 Stage the modified files:
  - `src/components/portal/inicio/InicioProximosEntrenamientos.tsx`
  - `src/components/portal/planes/SuscripcionModal.tsx`
- [ ] 7.2 Create a commit with message:
  ```
  ...
  Fixes: US-0072
  ```
- [ ] 7.3 Push branch to remote with `-u` flag
- [ ] 7.4 Create pull request with title: `fix(mobile-ui): make upcoming trainings and subscription modal responsive`
- [ ] 7.5 PR description should include:
...
- [ ] 7.6 Ensure PR passes any CI checks (linting, type checking, tests)
