...
`ValidarPagoModal` SHALL display the "Método" row using the value of `pago.metodo_pago_nombre` when non-null. When `metodo_pago_nombre` is null but the legacy `metodo_pago` text field is set, SHALL display that value. When both are null, SHALL display "—".
...
- **THEN** the "Método" row SHALL display "Transferencia Nequi"
...
- **THEN** the "Método" row SHALL display "transferencia"
...
- **THEN** the "Método" row SHALL display "—"
