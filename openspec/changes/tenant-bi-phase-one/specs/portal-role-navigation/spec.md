...
#### Scenario: Administrator sees analytics route entry
- **WHEN** the resolved role for active tenant is `administrador`
- **THEN** the sidebar SHALL include `Analítica` with `analytics` icon linking to `analitica` after `gestion-reservas`

#### Scenario: Non-administrator does not see analytics route entry
- **WHEN** the resolved role for active tenant is `usuario` or `entrenador`
- **THEN** the sidebar SHALL NOT include an entry for `analitica`
