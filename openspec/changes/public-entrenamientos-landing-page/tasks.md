...
- [ ] 8.2 **Requires live browser QA (not run this session — no browser automation tool available)**: click "Reservar" → `RegistrateParaReservarModal` opens; "Crear cuenta gratis" → `/auth/signup`; "Ya tengo cuenta" → `/auth/login?next=/portal/entrenamientos-publicos` → post-login lands on `/portal/entrenamientos-publicos`
- [ ] 8.3 **Requires live QA**: despublicar a training as admin through the running app UI, confirm it disappears from `/entrenamientos-publicos` on refresh
