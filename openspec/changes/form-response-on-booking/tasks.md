...
- [x] 2.1 Write `supabase/migrations/{timestamp}_formulario_respuestas.sql`: `formulario_respuestas` table, indexes, RLS (select-only policy), and the `create or replace function public.book_and_deduct_service_units(...)` extension (new `p_formulario_plantilla_id`/`p_formulario_respuesta` params, required-field validation, atomic response+reservation insert), per `design.md` decisions #1–#4
...
- [ ] 2.4 Manually verify in SQL: a response+reservation insert with all required fields succeeds; a missing required field raises `FORMULARIO_CAMPOS_FALTANTES`; a direct `insert into formulario_respuestas` from an authenticated role is rejected by RLS; deleting a referenced `formularios_plantillas` row fails with a FK violation — **blocked: depends on 2.3**
...
- [x] 6.2 `src/components/portal/entrenamientos/reservas/FormularioRespuestaViewerModal.tsx` (new): read-only "Ver respuesta" viewer — labels + values per section, signed-URL resolution for `imagen` fields, "Sin respuesta" fallback for unanswered/newer fields
...
- [ ] 8.8 Attempt to delete a `formularios_plantillas` row referenced by a response and confirm the friendly error message
