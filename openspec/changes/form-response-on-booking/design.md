...
**4. `formulario_plantilla_id` FK uses `on delete set null`, plus a `campos_snapshot` column captured at submission time.**
An admin must always be able to delete a `formularios_plantillas` row — even one with submitted responses — and the responses must survive that delete unchanged. `on delete set null` satisfies "can always delete"; but since template deletion is already a hard delete that cascades to `formulario_plantilla_esquema` (US-0084), the field labels/types would otherwise disappear along with it, leaving a response's JSON (keyed by opaque `campo_nombre` values like `"peso_kg"`) unreadable. `book_and_deduct_service_units` closes that gap by also writing `campos_snapshot` — `{ [campo_nombre]: { etiqueta, tipo, orden } }` for every active "datos" section — into `formulario_respuestas` at the moment the response is inserted. `FormularioRespuestaViewerModal` renders exclusively from this snapshot (never from the live template), so "Ver respuesta" looks identical whether the template still exists, was edited since, or was deleted entirely. Trade-off: the viewer can no longer show the template's `titulo`/`subtitulo`/`texto` sections for context (those aren't part of the snapshot, only "datos" fields are) — accepted, since the point of the viewer is reviewing an athlete's answers, not re-rendering the whole form.
Rejected alternative: keep `on delete restrict` and block deletion of any template with responses — this was the first cut of this design, reversed after the requester clarified that template deletion must never be blocked by response history.
...
- **[Risk]** `campos_snapshot` duplicates label/type data already present in `formulario_plantilla_esquema` at submission time — a classic denormalization staleness risk if the admin edits a field's label afterward. → **Mitigation**: this is intentional, not accidental — the snapshot is a point-in-time historical record by design (what the athlete actually saw when they answered), so it deliberately does NOT track later template edits.
