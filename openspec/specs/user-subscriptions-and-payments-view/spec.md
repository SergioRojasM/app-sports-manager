# Capability: user-subscriptions-and-payments-view

## Purpose
Defines the page and components allowing authenticated users with role `usuario` to view their own subscriptions and associated payment records for a tenant, upload or re-upload payment proof files, and filter the subscription list by status.
