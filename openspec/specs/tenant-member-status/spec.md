# Capability: tenant-member-status

## Purpose
Manages per-tenant membership status for athletes, providing an independent `estado` column on `miembros_tenant`, an immutable audit log of status changes, a flat database view enriched with recent-absence counts, an atomic RPC for status changes, and UI components enabling administrators to change member status and view their novelty history.
