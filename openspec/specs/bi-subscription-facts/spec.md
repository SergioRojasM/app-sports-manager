# bi-subscription-facts Specification

## Purpose
Defines `bi.fct_suscripciones`, the private subscription fact view (one row per subscription, with sale date, sold/active flags and a payment summary), and requires every subscription KPI in `get_tenant_bi_dashboard` to derive from it. "Active" means `estado = 'activa'`, the rule used across the app and kept current by the daily expiry cron.
## Requirements
