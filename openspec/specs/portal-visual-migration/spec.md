# Capability: portal-visual-migration

## Purpose
Requires every `/portal/*` screen to use only the `grit-*` tokens and the `ui/grit` kit — no legacy tokens, no overridden `rounded-lg`/`rounded-xl` radii — with consistent page scaffolding, modal styling and inputs, and without changing any behaviour.
