# analitica-detail-tabs Specification

## Purpose
Defines the content of the Ingresos, Operación and Equipo tabs of `/portal/orgs/{tenant_id}/analitica`: KPI rows, monthly charts, tables with column headers and rankings, their metric definitions (total = validated + pending, per-training averages, active athletes) and the `get_tenant_bi_dashboard` fields that feed them. Equipo considers athletes only.
## Requirements
