# analitica-resumen-dashboard Specification

## Purpose
Defines the content of the "Resumen" tab of `/portal/orgs/{tenant_id}/analitica`: two rows of four KPI cards (revenue, trainings, per-training bookings/occupancy/attendance, athletes) and three rows of `@nivo` charts (monthly revenue, validated vs pending, subscriptions sold per month and per plan, average bookings per training, bookings by discipline), their metric definitions, and the additive `get_tenant_bi_dashboard` fields that feed them.
## Requirements
