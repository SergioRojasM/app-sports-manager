# Capability: admin-payment-detail-view

## Purpose
Defines the read-only payment detail modal (`VerDetallePagoModal`) accessible to administrators from the subscription management table, allowing them to inspect full payment information and comprobante (proof of payment) without any mutation actions.
