# Capability: public-training-detail-visual

## Purpose
Defines the visual contract of the public training detail page (`/entrenamientos-publicos/[entrenamiento_id]`): its breadcrumb row and content sections follow design node `OyIqr`, while the landing `Header`/`Footer` and every existing behaviour stay unchanged.
